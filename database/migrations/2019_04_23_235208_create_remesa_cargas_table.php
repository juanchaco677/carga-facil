...
class CreateRemesaCargasTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('remesa_cargas', function (Blueprint $table) {
            ...
            $table->string('id_empresa_destinatario',8);
            $table->foreign('id_empresa_destinatario')->references('nit')->on('empresas');
            $table->string('id_empresa_transporta',8);
            $table->foreign('id_empresa_transporta')->references('nit')->on('empresas');
            $table->string('id_empresa_transporta',8);
            $table->foreign('id_empresa_transporta')->references('nit')->on('empresas');
            $table->primary('id');
            $table->timestamps();
